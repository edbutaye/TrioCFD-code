/*
 * Intersections_Arete_Facettes_Data.cpp
 *
...
#include <Intersections_Arete_Facettes_Data.h>
                                                       ...
                                                       double barycentre_w)
{
  ...
  // chercher la fin...)
  {
    ...
    new_entry.numero_arete_ = num_face;
...
Intersections_Arete_Facettes::Intersections_Arete_Facettes()
                                         ...
                                         int nb_facettes)
{
  ...
  index_arete_facette_ = -1;
  index_facette_arete_.resize_array(nb_facettes);
                                                               ...
                                                               ArrOfInt& liste_faces) const
{
  ...
  if (num_facette >= index_facette_arete_.size_array())
...
const ArrOfInt& Intersections_Arete_Facettes::index_arete() const
{
  return index_arete_facette_;
}
...
const ArrOfInt& Intersections_Arete_Facettes::index_facette() const
{
  return index_facette_arete_;
}

// Description:
//operateur de copie
const Intersections_Arete_Facettes& Intersections_Arete_Facettes::operator=(const Intersections_Arete_Facettes& iaf)
{
  if (&iaf != this)
    {
      index_arete_facette_ = iaf.index_arete();
      assert(index_arete_facette_.size_array()>0);
      index_facette_arete_ = iaf.index_facette();
      data_allocated_size = iaf.data_allocated_size;
      data_real_size = iaf.data_real_size;
