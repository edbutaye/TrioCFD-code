...
\begin{itemize}
    ...
    \item rapide explication du fonctionnement des classes : sources génériques (i.e. classe base) dans src/Multiphase/CMFD/Turbulence/Sources et spécialisation par discrétisation dans src/Multiphase/CMFD/Turbulence/VDF/Sources et src/Multiphase/CMFD/Turbulence/PolyMAC_MPFA/Sources
...
\end{align}
It is derived directly from Source_base and currently only implemented in PolyMAC_MPFA.

The added production term in the turbulent kinetic energy equation is defined in Production_HZDR_PolyMAC_MPFA. The added term in the dissipation equation is defined in Source_Dissipation_HZDR_PolyMAC_MPFA. TODO: To avoid code duplication, the dissipation source term should call the production source term.
...
\begin{itemize}
    ...
    \item Dissipation_WIT_PolyMAC_MPFA
    \item Production_WIT_PolyMAC_MPFA
\end{itemize}

As specified above, it is currently only available with the PolyMAC\_MPFA discretisation. To take the WIT into account in the momentum equation, one must specify its presence in the diffusion term using\footnote{This syntax might evolve to avoid repeating the names.}
