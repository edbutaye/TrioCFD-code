...
For example, in \cite{KREPPER20113851}, one can find : \\
\begin{center}
\begin{tabular}{c c c }
\toprule
...
The model is described in \cite{IshiiZuber}.\\
The model is implemented in:
\begin{lstlisting}[language=c++]
...
The model is also described in \cite{IshiiZuber}.\\
...
The model is described in \cite{Tomiyama2002}.\\
The model is implemented in :
\begin{lstlisting}[language=c++]
...
The model is described in \cite{ANTAL1991635}.\\
The model is implemented in :
\begin{lstlisting}[language=c++]
void Correction_Antal_PolyMAC_MPFA::set_param(Param& param)
{
...
\begin{lstlisting}[language=c++]
void Correction_Lubchenko_PolyMAC_MPFA::set_param(Param& param)
{
...
The model is also described in \cite{}.\\
The model is implemented in :
\begin{lstlisting}[language=c++]
...
The model is implemented in:
\begin{lstlisting}[language=c++]
void Injection_QDM_nulle_PolyMAC_MPFA::set_param(Param& param)
{
  Param param(que_suis_je());
  param.ajouter("beta", &beta_);
  param.lire_avec_accolades_depuis(is);
...
\end{equation}
The {\colorbox{codebackground}{\color{codekeyword3} Density\ change}} is implemented in \texttt{Variation_rho_Elem_PolyMAC_MPFA} :
\begin{lstlisting}[language=c++]
...
The model is described in \cite{YAO2004307}.
...
\begin{lstlisting}[language=c++]
void Rupture_bulles_1groupe_PolyMAC_MPFA::set_param(Param& param)
{
  Param param(que_suis_je());
  param.ajouter("beta_k", &beta_k_);
}
...
The {\colorbox{codebackground}{\color{codekeyword3}  Nucleation}} model is implemented in Nucleation_paroi_PolyMAC_MPFA:
\begin{lstlisting}[language=c++]
void Nucleation_paroi_PolyMAC_MPFA::set_param(Param& param)
\end{lstlisting}
...
\begin{itemize}
\item[\small \textcolor{blue}{\ding{109}}] The \cite{SUN2} model was developed for a $2$ group configuration with a $200 \times  10$ $mm^2$ confined rectangular channel data. The effect of the wall is then very significant. It was performed for liquid superficial velocity between $0.32$ and $2.84$ m/s and gas velocity between $0.39$ and $2.01$ m/s. It deals with cap-bubbly and churn-turbulent flows.\\
\item[\small \textcolor{blue}{\ding{109}}] The \cite{Smith1} model was developed for a $2$ group configuration with $0.102$ mm and $0.152$ mm diameter pipes. It deals with bubbly, cap-bubbly and churn-turbulent flows. \\
\item[\small \textcolor{blue}{\ding{109}}] The \cite{Schlegel1} model was developed for a $2$ group configuration with large diameter channels. It deals with bubbly and cap-bubbly flows. Several constitutive relations and correlations were used to tune this model.\\
\item[\small \textcolor{blue}{\ding{109}}] The \cite{Fu2002} model was developed for a $2$ group configuration for small round pipe.\\
\item[\small \textcolor{blue}{\ding{109}}] \cite{Dave2016a} proposed new Smith coefficient based on optimization with genetic algorithm on all TOPFLOW DN200 (pipe $195.3$ mm).
\end{itemize}
\begin{figure}[!ht]
    \centering
