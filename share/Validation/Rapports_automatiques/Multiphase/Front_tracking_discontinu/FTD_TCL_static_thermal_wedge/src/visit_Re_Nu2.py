...
list_ts = list(range(ts_start, int(ts_end), nb_ts))  # On centralise ici la liste des timestep e traiter.
print(list_ts)
pi = math.pi
...
if GRADT == 1:
    DefineScalarExpression("Grad_T_fois_surf_elem",
                           "pos_cmfe(<[0]id:GRAD_TEMP_THERMIQUE_ELEM_dom>,INTERFACES,-100.)*sinterf")
else:
    ...
    DefineScalarExpression("Grad_T_fois_surf_elem", str(L_vap) + "/" + str(
        k_l) + "*pos_cmfe(<[0]id:MPOINT_THERMIQUE_ELEM_dom>,INTERFACES,-100.)*sinterf")

if PAR == 1:
    DefineScalarExpression("facettes_locales",
                           "eq(PE_LOCAL_ELEM_INTERFACES, PE_ELEM_INTERFACES)")  # Pour eliminer facettes virtuelles
...
for i in range(nb_max_authorized_bubbles):
    ...
    del f
    ff[i].write(
        "# 1-index |  2-Time  |  3-S(t)  |  4-S(t)/S(0)  | 5-S(t)/S_sphere(t) |  6-V(t)  |  7-V(t)/V(t=0)  | 8-D_eq  | 9-Vit_vap_x  | 10-Vit_vap_y  | 11-Vit_vap_z  | 12-Norme_Vit_vap  | 13-Re_b=Norme_Vit_vap*D_eq/nu_l  | 14-Nusselt | 15-x_g | 16-y_g | 17-z_g  \n")
    pass
...
for state in list_ts:
    ...
    DrawPlots() #Utile si on tape le continue
    ...
    for bulle in lbulles:
        ...
        print(
            "Le volume (resp. surf) de la bulle est de %g m^3 (resp m^2), la surf equiv (resp. perimetre) est %g et le diametre equivalent est de %g m" % (
            volume_bulle, surface_equiv, diam_bulle_equiv))
        # ------------------------------------
        #         Pour la thermique
        # ------------------------------------
