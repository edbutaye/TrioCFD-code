...
seq $nelem | awk 'BEGIN{x=ENVIRON["connex_compo"]}{print x}'>>$compofile
  ...
  echo Champ SOMMETS $(basename $nodefile) geometrie=FTMESH size=$nnodestot composantes=3 >>$lata
  echo Champ ELEMENTS $(basename $elemfile) geometrie=FTMESH size=$nelemtot composantes=3 FORMAT=INT32 >>$lata
  echo Champ COMPO_CONNEXE $(basename $compofile) geometrie=FTMESH size=$nelemtot composantes=1 FORMAT=INT32,NO_INDEXING localisation=ELEM >>$lata
  echo FIN >>$lata
}
exit
